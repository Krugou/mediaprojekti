...
function tyhjenna(){
    try{
        ...
        document.body.appendChild(uusiElementti);
...
async function haeValittuRanta(evt){
    ...
    try {
        document.getElementById("tulos").innerHTML += data.meta.name + " tiedot:<br>";
    } catch(error) {
        console.log("Valitun rannan nimeä ei löytynyt");
    }
    try {
        document.getElementById("tulos").innerHTML += "Veden lämpötila on " + data.data[data.data.length - 1].temp_water + "°C<br>";
    } catch (error) {
        console.log("Valitun rannan veden lämpötilaa ei löytynyt");
    }
    try {
        document.getElementById("tulos").innerHTML += "Ilman lämpötila on " + data.data[data.data.length - 1].temp_air + "°C<br>";
    } catch (error) {
        console.log("Valitun rannan ilman lämpötilaa ei löytynyt");
    }
    try {
        document.getElementById("tulos").innerHTML += "Mitattu ajassa " + data.data[data.data.length - 1].time + "<br>";
    } catch(error) {
